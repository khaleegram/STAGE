...
import { importColleges } from './actions';
import { Card } from '@/components/ui/card';

interface ImportCollegesModalProps {
  isOpen: boolean;
  onClose: () => void;
}

const CSV_TEMPLATE_HEADERS = "name,code\n";
const CSV_TEMPLATE_ROW1 = "COLLEGE OF NATURAL AND APPLIED SCIENCES,NAS\n";
const CSV_TEMPLATE_ROW2 = "COLLEGE OF HUMANITIES,HUM\n";

export function ImportCollegesModal({ isOpen, onClose }: ImportCollegesModalProps) {
  ...
  const handleImport = async () => {
    ...
    const result = await importColleges(parsedData);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-4xl">
        <DialogHeader>
          <DialogTitle>Import Colleges from CSV</DialogTitle>
          <DialogDescription>
            Upload a CSV file with college details to bulk-add them to the system.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <Card className="bg-muted/50 p-4">
              <p className="text-sm text-muted-foreground">
                Your CSV must contain these headers: <code className="font-mono bg-background p-1 rounded-sm">name</code> and <code className="font-mono bg-background p-1 rounded-sm">code</code>.
              </p>
