...
interface ImportLevelsModalProps {
  isOpen: boolean;
  onClose: () => void;
  programs: Program[];
}

const CSV_TEMPLATE_HEADERS = "level,students_count\n";
const CSV_TEMPLATE_ROW1 = "1,150\n";
const CSV_TEMPLATE_ROW2 = "2,135\n";

export function ImportLevelsModal({ isOpen, onClose, programs }: ImportLevelsModalProps) {
  ...
  const handleDownloadTemplate = () => {
    ...
    link.setAttribute("download", "levels_template.csv");
  ...
  const handleImport = async () => {
    if (!selectedProgramId) {
      setError("Please select a program to import levels into.");
      return;
    }
    ...
    const result = await importLevels(selectedProgramId, parsedData);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-4xl">
        <DialogHeader>
          <DialogTitle>Import Levels from CSV</DialogTitle>
          <DialogDescription>
            Select a program, then upload a CSV file with level details to bulk-add them.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <Card className="bg-muted/50 p-4">
              <p className="text-sm text-muted-foreground">
                Your CSV must contain these headers: <code className="font-mono bg-background p-1 rounded-sm">level</code> and <code className="font-mono bg-background p-1 rounded-sm">students_count</code>.
              </p>
