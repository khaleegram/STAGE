...
import { importDepartments } from './actions';
...
const CSV_TEMPLATE_HEADERS = "name,college_name\n";
const CSV_TEMPLATE_ROW1 = "Computer Science,COLLEGE OF NATURAL AND APPLIED SCIENCES\n";
const CSV_TEMPLATE_ROW2 = "History,COLLEGE OF HUMANITIES\n";

export function ImportDepartmentsModal({ isOpen, onClose, colleges }: ImportDepartmentsModalProps) {
  ...
  const handleImport = async () => {
    ...
    const result = await importDepartments(parsedData, plainColleges);
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-4xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload a CSV file with department details to bulk-add them to the system.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <Card className="bg-muted/50 p-4">
              <p className="text-sm text-muted-foreground">
                Your CSV must contain these headers: <code className="font-mono bg-background p-1 rounded-sm">name</code> and <code className="font-mono bg-background p-1 rounded-sm">college_name</code>. The college name must match an existing college exactly.
              </p>
